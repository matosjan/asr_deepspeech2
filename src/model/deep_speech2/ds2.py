...
class DeepSpeech2(nn.Module):
    def __init__(self, n_feats, n_tokens, gru_layers_num=7, gru_hidden_size=512) -> None:
        super().__init__()

        assert gru_layers_num > 0

        self.conv_layers = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=32, kernel_size=(41, 11), stride=(2, 2), padding=(20, 5), bias=False),
            nn.BatchNorm2d(num_features=32),
            nn.Hardtanh(0, 20),

            nn.Conv2d(in_channels=32, out_channels=32, kernel_size=(21, 11), stride=(2, 1), padding=(10, 5), bias=False),
            nn.BatchNorm2d(num_features=32),
            nn.Hardtanh(0, 20),

            nn.Conv2d(in_channels=32, out_channels=96, kernel_size=(21, 11), stride=(2, 1), padding=(10, 5), bias=False),
        ...
        for layer in self.conv_layers:
            if isinstance(layer, nn.Conv2d):
                gru_input_size = after_conv(n_features=gru_input_size, conv=layer, dim=0)
        gru_input_size *= 96

        self.first_gru = GRUBlock(gru_input_size=gru_input_size, gru_hidden_size=gru_hidden_size, batch_norm=False)
        self.gru_layers = nn.ModuleList([GRUBlock(gru_input_size=gru_hidden_size, gru_hidden_size=gru_hidden_size) for _ in range(gru_layers_num - 1)])
